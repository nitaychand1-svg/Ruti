...
class EnsembleTrainer:
    ...
    async def train(self, X: np.ndarray, y: np.ndarray,
        ...
        if oos_score < 0.55:
            self.logger.warning(f"[FIX-5] OOS validation low: {oos_score:.3f}")
    ...
    def _train_blender(self, base_models: List[Dict], 
        ...
        blender_input = self._prepare_blender_input(base_models, X, y)
    ...
    def _prepare_blender_input(self, base_models: List[Dict], X: np.ndarray, y: np.ndarray):
    ...
    def _calculate_correlation_matrix(self, models: List[Dict], X: np.ndarray):
        ...
        predictions = np.column_stack([
            m['model'].predict_proba(X_sample)[:, 1] for m in models
        ])

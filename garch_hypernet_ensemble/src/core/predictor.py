...
class EnsemblePredictor:
    ...
    async def _get_base_predictions(self, models: list, X: np.ndarray):
        ...
        if predictions and isinstance(predictions[0], np.ndarray):
            predictions_array = np.column_stack(predictions) if len(predictions) > 1 else np.array(predictions).T
        else:
            predictions_array = np.array(predictions).T
        
        return predictions_array, confidences, model_ids
    ...
    def _fallback_rule_based(self, X: np.ndarray) -> Dict[str, Any]:
        ...
        ma_short = np.mean(X[-5:, 0])
        ma_long = np.mean(X[-20:, 0])
    ...
    def _prepare_meta_features(self, diversity: float, avg_correlation: float,
        ...
        return np.array([
            ...
            regime_features[4],  # Chaotic membership
            garch_vol,
            garch_forecast,
    ...
    def _encode_regime_continuous(self, X: np.ndarray, vol: float):
        ...
        vol_percentile = np.clip(
            vol / (np.percentile(np.abs(returns), 95) + 1e-6), 0, 1
        )
        trend_strength = np.abs(np.mean(returns[-20:])) / (np.std(returns) + 1e-6)
        momentum = np.mean(prices[-10:]) - np.mean(prices[-50:]) if len(prices) >= 50 else 0.0
    ...
    def _calculate_position_size(self, ensemble_pred: np.ndarray, 
        ...
        # Kelly
        odds = (p_up / (1 - p_up)) if p_up > 0.5 else ((1 - p_up) / p_up)
    ...
    def _prepare_blender_input(self, base_preds: np.ndarray, 
        ...
        stats = np.column_stack([
            ...
            np.median(base_preds, axis=1),
            np.std(base_preds, axis=1),
            np.percentile(base_preds, 25, axis=1),
            np.percentile(base_preds, 75, axis=1),
            np.percentile(base_preds, 95, axis=1),
            np.percentile(base_preds, 5, axis=1),
        ])
        
        features = X[-1].reshape(1, -1) if X.ndim > 1 else X.reshape(1, -1)
        
        # Ensure base_preds has enough columns
        if base_preds.shape[1] >= 10:
            preds_subset = base_preds[:10]
        else:
            preds_subset = np.pad(base_preds, ((0, 0), (0, 10 - base_preds.shape[1])), mode='constant')
        
        return np.column_stack([features, stats, preds_subset, np.array([vol])])

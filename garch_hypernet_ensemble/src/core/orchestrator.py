...
class GARCHHyperAdaptiveEnsemble:
    ...
    async def predict(
        ...
        return await self.predictor.predict(X, market_regime, ground_truth, vix)
    ...
    async def online_update(self, X: np.ndarray, y: int, regime: str) -> None:

...
class AdaptiveHyperNetwork(Model):
    ...
    def __init__(self, meta_dim: int, n_models: int, 
        ...
        for i, dim in enumerate(hidden_dims):
            self.hidden_blocks.extend([
                layers.Dense(dim, kernel_regularizer=l2(0.01), 
                            name=f'hyper_dense_{i}'),
        ...
        self.weight_generator = layers.Dense(
            n_models, activation=None, name='weight_logits'
        )
        self.threshold_generator = layers.Dense(
            3, activation='sigmoid', name='threshold_gen'
        )
    
    def call(self, meta_features, training=False):
        ...
        # [FIX-4] Entropy regularization for diversity
        if training:
            ...
            weighted_corr = tf.matmul(
                weights, tf.matmul(corr_matrix, tf.transpose(weights))
            )
    ...
    def update_correlation_matrix(self, corr_matrix: np.ndarray):

...
class StressTestingModule:
    ...
    async def run_monte_carlo_stress(self, X: np.ndarray, y: np.ndarray,
        ...
        for X_stress, y_stress in scenarios[:100]:
            ...
            results['correlation_spikes'].append(1.0 - pred_result['diversity'])
            results['diversity_scores'].append(pred_result['diversity'])
            ...
            if pred_result['diversity'] < results['worst_case_diversity']:
        ...
        # Summary statistics
        results['mean_correlation'] = np.mean(results['correlation_spikes']) if results['correlation_spikes'] else 0.0
        results['diversity_at_risk'] = np.percentile(results['diversity_scores'], 5) if results['diversity_scores'] else 0.0
        results['mean_garch_vol'] = np.mean(results['garch_spikes']) if results['garch_spikes'] else 0.0
        results['black_swan_diversity'] = np.min(results['black_swan_scenarios']) if results['black_swan_scenarios'] else 0.0
        
        # [FIX-5] Threshold check
        min_diversity = getattr(self.ensemble.config.monitoring, 'min_diversity_threshold', 0.25)
    ...
    async def _run_black_swan_scenarios(self) -> List[float]:
        ...
        for _ in range(100):
            ...
            pred_swan = await self.ensemble.predict(
                X_swan, market_regime='chaotic'
            )
            results.append(pred_swan['diversity'])

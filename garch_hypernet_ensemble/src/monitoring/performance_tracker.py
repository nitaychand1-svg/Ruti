...
class OnlinePerformanceTracker:
    ...
    def update_metrics(self, prediction: np.ndarray, 
        ...
        if hasattr(self, 'backtest_score'):
            gap = abs(np.mean(self.accuracy_window) - self.backtest_score) if self.accuracy_window else 0.0
            self.live_vs_backtest_gap.append(gap)
        
        # [FIX-5] Auto-tune thresholds
        self._auto_tune_thresholds(regime, avg_corr if correlation_matrix.size > 1 else 0.0, diversity)

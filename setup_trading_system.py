...
# ??????? ?????????? ???????
BASE_DIR = Path("/storage/emulated/0/trading_system")
...
def main():
    ...
    # .sops.yaml
    write_file(BASE_DIR / ".sops.yaml", """creation_rules:
...
""")
    ...
    # Dockerfile
    write_file(BASE_DIR / "Dockerfile", """FROM python:3.13-slim
...
""")
    ...
    write_file(BASE_DIR / "all_files_content.txt", "")
    
    # .github/workflows/build_and_push.yaml
    write_file(BASE_DIR / ".github" / "workflows" / "build_and_push.yaml", """name: Build and Push Trading System
...
""")
    ...
    # app/main.py
    write_file(BASE_DIR / "app" / "main.py", """import os
import time
import uuid
...
""")
    ...
    # app/config.yaml
    write_file(BASE_DIR / "app" / "config.yaml", """vault:
...
""")
    ...
    # app/tests/test_debate_tasks.py
    write_file(BASE_DIR / "app" / "tests" / "test_debate_tasks.py", """import pytest
import asyncio
...
""")
    ...
    # app/tasks/debate_tasks.py
    write_file(BASE_DIR / "app" / "tasks" / "debate_tasks.py", """from app.modules.taskgraph import Node, TaskGraph
...
""")
    ...
    write_file(BASE_DIR / "app" / "modules" / "logging_config.py", """import logging, sys
...
""")
    ...
    # app/modules/taskgraph.py
    write_file(BASE_DIR / "app" / "modules" / "taskgraph.py", """import asyncio
...
""")
    ...
    # app/modules/cognitive_middleware.py
    write_file(BASE_DIR / "app" / "modules" / "cognitive_middleware.py", """class CognitiveMiddleware:
    @staticmethod
...
""")
    ...
    # app/modules/llm_wrapper.py
    write_file(BASE_DIR / "app" / "modules" / "llm_wrapper.py", """import asyncio
...
""")
    ...
    # app/modules/rl_agent.py
    write_file(BASE_DIR / "app" / "modules" / "rl_agent.py", """import random
...
""")
    ...
    # app/modules/observability.py
    write_file(BASE_DIR / "app" / "modules" / "observability.py", """from opentelemetry import trace
...
""")
    ...
    write_file(BASE_DIR / "app" / "modules" / "data_sources.py", """import asyncio
...
""")
    ...
    write_file(BASE_DIR / "app" / "modules" / "api_routes.py", """from fastapi import APIRouter, Request, Query
from app.tasks.debate_tasks import create_debate_graph
import asyncio
from app.modules.logging_config import logger
...
""")
    ...
    # app/tests/test_taskgraph.py
    write_file(BASE_DIR / "app" / "tests" / "test_taskgraph.py", """import pytest
import asyncio
...
""")
    ...
    write_file(BASE_DIR / "app" / "__init__.py", "")
    write_file(BASE_DIR / "app" / "modules" / "__init__.py", "")
    write_file(BASE_DIR / "app" / "tasks" / "__init__.py", "")
    write_file(BASE_DIR / "app" / "tests" / "__init__.py", "")
    
    # k8s/vault-policy.hcl
    write_file(BASE_DIR / "k8s" / "vault-policy.hcl", """path "secret/data/trading/*" {
  capabilities = ["create", "read", "update", "delete", "list"]
}
""")
    
    # k8s/network_policy.yaml
    write_file(BASE_DIR / "k8s" / "network_policy.yaml", """apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
...
""")
    ...
    # k8s/deployment_security.yaml
    write_file(BASE_DIR / "k8s" / "deployment_security.yaml", """apiVersion: apps/v1
kind: Deployment
metadata:
  name: trading-system
spec:
...
""")
    ...
    # k8s/vault-rotation-job.yaml
    write_file(BASE_DIR / "k8s" / "vault-rotation-job.yaml", """apiVersion: batch/v1
kind: CronJob
metadata:
  name: vault-rotation
  namespace: trading-system
...
""")
    ...
    # requirements.txt
    write_file(BASE_DIR / "requirements.txt", """fastapi==0.104.1
uvicorn==0.24.0
...
""")
    ...
    # README.md
    write_file(BASE_DIR / "README.md", """# Trading System

Trading system with FastAPI, OpenTelemetry, and Kubernetes deployment.
